...
struct Example<A: hal::Api> {
    ...
    texture: A::Texture,
    instances: [AccelerationStructureInstance; 1],
...
impl<A: hal::Api> Example<A> {
    fn init(window: &winit::window::Window) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let aabb_buffer = unsafe {
            ...
            let mapping = device
                .map_buffer(&aabb_buffer, 0..aabb_size_in_bytes as u64)
                .unwrap();
            ptr::copy_nonoverlapping(
                aabb.as_ptr() as *const u8,
                mapping.ptr.as_ptr(),
                aabb_size_in_bytes,
            );
            device.unmap_buffer(&aabb_buffer);
            assert!(mapping.is_coherent);

            aabb_buffer
        };

        let blas_aabbs = vec![hal::AccelerationStructureAABBs {
...
fn main() {
    ...
    let window = winit::window::WindowBuilder::new()
        .with_title("hal-ray-traced-sphere")
