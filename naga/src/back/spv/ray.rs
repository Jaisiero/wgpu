...
impl<'w> BlockContext<'w> {
    ...
    pub(super) fn write_ray_query_get_candidate_intersection(
        &mut self,
        query: Handle<crate::Expression>,
        block: &mut Block,
    ) -> spirv::Word {
        ...
        let intersection_id = self.writer.get_constant_scalar(crate::Literal::U32(
            spirv::RayQueryIntersection::	
            RayQueryCandidateIntersectionKHR as _,
        ));
    ...
    pub(super) fn write_ray_query_generate_intersection(
        &mut self,
        query: Handle<crate::Expression>,
        hit: Handle<crate::Expression>,
        block: &mut Block,
    )  -> spirv::Word {
        let query_id = self.cached[query];
        let hit_id = self.cached[hit];
        let id = self.gen_id();
        let result = self
            .ir_module
            .special_types
